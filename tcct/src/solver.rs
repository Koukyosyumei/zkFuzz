...
pub fn adaptive_search(
    ...
    fn search_with_range(
        ...
        current_iteration: &Arc<AtomicUsize>,
        progress_interval: usize,
        range: &(BigInt, BigInt), // 範囲 (start, end)
    ) -> VerificationResult {
        if index == variables.len() {
            let iter = current_iteration.fetch_add(1, Ordering::SeqCst);
            if iter % progress_interval == 0 {
                print!("\rProgress: {} iterations", iter);
                io::stdout().flush().unwrap();
            }
        ...
        let var = &variables[index];
        let mut value = range.0.clone();
        while value < range.1 {
            assignment.insert(var.clone(), value.clone());
            let result = search_with_range(
                prime,
                id,
                sexe,
                index + 1,
    ...
    let mut range_start = BigInt::zero();
    let mut range_end = initial_range.clone();
    loop {
        println!("Trying range: [{}, {})", range_start, range_end);
        let flag = search_with_range(
            &prime,
            &id,
            sexe,
            0,
        ...
        range_end = range_end.clone() * BigInt::from(2);
        if range_start >= prime {
            break;
        }
        range_end = range_end.min(prime.clone());
    }
